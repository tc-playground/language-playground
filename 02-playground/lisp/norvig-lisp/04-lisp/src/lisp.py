...
def eval_sexpr(sexpr, env=GLOBAL_ENV):
    ...
    # constant literal
        ...
        return sexpr
    # quotation
    elif sexpr[0] == SYMBOL_TABLE["quote"]:
        (_, exp) = sexpr
        return exp
    # conditional
    elif sexpr[0] == SYMBOL_TABLE["if"]:
        (_, test, conseq, alt) = sexpr
        exp = (conseq if eval_sexpr(test, env) else alt)
        return eval_sexpr(exp, env)
    # definition
    elif sexpr[0] == SYMBOL_TABLE["define"]:
        (_, var, exp) = sexpr
        env[var] = eval_sexpr(exp, env)
    # assignment
    elif sexpr[0] == SYMBOL_TABLE["set!"]:
        (_, var, exp) = sexpr
        env.find(var)[var] = eval_sexpr(exp, env)
    # procedure
    elif sexpr[0] == SYMBOL_TABLE["lambda"]:
        (_, params, body) = sexpr
