...
% Define location of complex object objects location_s(Thing, Place).
% NB: we could still have used the location/2 predicate...
...
list_things_s(Place) :-  
    ...
    write('A '),write(Size),tab(1), write(Color),tab(1), write(Thing), 
    write(', weighing '), write(Weight), /*write(' pounds'), */ nl,
    fail.
list_things_s(_).
